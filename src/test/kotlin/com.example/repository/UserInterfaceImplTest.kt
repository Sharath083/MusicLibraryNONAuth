...
class UserInterfaceImplTest  {
    ...
    private val playList=AddToPlayList("731d67a1-b7d4-4eb6-ade2-93f835a8a457","cold", "my1")
    private val removePlayList=RemoveFromPlayList("731d67a1-b7d4-4eb6-ade2-93f835a8a457","cold", "my1")
    ...
    fun filterByArtistTest() : Unit = runBlocking{
        ...
        userInterfaceImpl.filterByArtist(input.artist!!).apply {
            assertEquals("cold",this[0].tittle)
            assertNotEquals("abc",this[0].tittle)
        }
    }
    @Test
    fun getSongIdTest():Unit= runBlocking {
        adminInterfaceImpl.addSong(input)
        transaction {
            userInterfaceImpl.getSongId(input.tittle!!).apply {
                assertTrue(this.toString().isNotEmpty())
            }
            try {
                userInterfaceImpl.getSongId("abc")
            }catch (e:SongNotFoundException){
                assertEquals("Song Does Not Exists",e.msg)
    ...
    fun checkSongInDbTest():Unit= runBlocking{
        adminInterfaceImpl.addSong(input)
        userInterfaceImpl.checkSongInDb(input.tittle!!).apply {
            assertTrue(this)
        }
...
//        }
    ...
    fun checkSongInPlayListTest():Unit= runBlocking {
        ...
        userInterfaceImpl.checkSongInPlayList(input.tittle!!,playList.playList!!,uuid).apply {
            assertFalse(this)
        }
